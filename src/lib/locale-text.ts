...
const BILINGUAL_TABLE: Array<[string, string]> = [
  ...
  // runtime_guard.rs — approval titles
  ...
  // runtime_guard.rs — approval summaries (partial match fragments)
  ['想启动。第一次运行前，AgentShield 需要先确认你是否愿意放行。', 'wants to launch. AgentShield needs your approval before first run.'],
  ['想发起一次联网操作，正在等你决定。', 'wants to make a network request. Waiting for your approval.'],
  ['想发起一次数据库外联或外传操作，正在等你决定。', 'wants to initiate a database exfiltration. Waiting for your approval.'],
  ['触发了一个需要你确认的敏感操作。', 'triggered a sensitive action that needs your approval.'],
  ['想把内容提交到网页。AgentShield 正在等你确认目标与内容。', 'wants to submit content to a web page. Waiting for your approval.'],
  ['想提交支付请求。AgentShield 正在等你确认金额与目标。', 'wants to submit a payment. Waiting for your approval.'],
  ['想发送邮件。AgentShield 正在等你确认收件人与正文。', 'wants to send an email. Waiting for your approval.'],
  ['想删除或归档邮件。AgentShield 正在等你确认范围。', 'wants to delete or archive emails. Waiting for your approval.'],
  ['想执行命令。AgentShield 正在等你确认命令内容。', 'wants to execute a command. Waiting for your approval.'],
  ['想把数据库内容或数据库连接流量发往外部目标。AgentShield 正在等你确认目标与范围。', 'wants to send database content to an external target. Waiting for your approval.'],
  ['在你点头前，这次安装不会被放行。', 'Installation blocked until you approve.'],
  ['在你点头前，这次删除不会被放行。', 'Deletion blocked until you approve.'],
  ['在你点头前，AgentShield 会继续拦住。', 'Blocked until you approve.'],
  ['在你点头前，明文不会被取出。', 'Plaintext will not be revealed until you approve.'],
  ['在你点头前，这个地址不会被加入允许名单。', 'This address will not be allowed until you approve.'],
  ['正在等你决定。', 'Waiting for your approval.'],
  ['想把新的扩展能力写入', 'wants to install extensions to'],
  ['想删除', 'wants to delete'],
  ['想批量改动', 'wants to bulk modify'],
  ['想删除密钥', 'wants to delete credential'],
  ['想显示或导出密钥', 'wants to export credential'],
  ['想连接', 'wants to connect to'],
  ['想把数据库流量或数据发往', 'wants to send database traffic to'],

  // runtime_guard.rs — consequence lines
  ['一旦放行，AgentShield 会把该扩展能力写入你选中的工具配置。', 'Once approved, AgentShield will write this extension to your selected tool config.'],
  ['一旦放行，目标文件或配置可能会被直接删除。', 'Once approved, the target files or configs may be permanently deleted.'],
  ['一旦放行，多个文件或配置会被同时改写，回滚会更困难。', 'Once approved, multiple files or configs will be modified simultaneously. Rollback will be harder.'],
  ['一旦放行，密钥会从 AgentShield 保险库和系统钥匙串中删除。', 'Once approved, the credential will be removed from the vault and system keychain.'],
  ['一旦放行，密钥会以明文形式暴露到剪贴板或界面。', 'Once approved, the credential will be exposed in plaintext to the clipboard or UI.'],
  ['一旦放行，内容会被提交到外部网页，可能包含敏感信息。', 'Once approved, content will be submitted to an external web page and may contain sensitive data.'],
  ['一旦放行，可能触发真实支付或订阅扣费。', 'Once approved, this may trigger a real payment or subscription charge.'],
  ['一旦放行，邮件会真实发出。', 'Once approved, the email will actually be sent.'],
  ['一旦放行，邮件可能被删除、归档或移动。', 'Once approved, emails may be deleted, archived, or moved.'],
  ['一旦放行，命令会在本机真实执行。', 'Once approved, the command will be executed on this machine.'],
  ['一旦放行，数据库记录、查询结果或连接流量可能被发送到外部目标。', 'Once approved, database records or traffic may be sent to an external target.'],
  ['如果你现在不点允许，这次操作会继续被拦住，不会自动放行。', 'If you do not approve now, this action will remain blocked and will not proceed automatically.'],
  ['高危动作缺少精确目标，默认继续拦住。', 'High-risk action is missing a precise target. Blocked by default.'],
